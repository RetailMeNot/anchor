...
#### fonts
The CSS property, `font-family` associates to these keys.
...
#### radii
The CSS property, `border-radius` associates to these keys.
...
xstyled's <a href="https://xstyled.dev/docs/migrate-from-styled-components/#css-utility-is-required" target="_blank">`css()`</a> function
(refer to their documentation for more information). To put in new styles, use the `css()`
function yourself.
...
```jsx
    ...
    const Example = () => (
        <section>
            <Typography tag="h2">Welcome!</Typography>
            <Typography tag="p">Lorem Ipsum etc, etc...</Typography>
        <section>
    );
