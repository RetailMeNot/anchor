...
export const ResponsiveContextExample = () => {
    ...
    return (
        <StyledResponsiveContextExample>
            <Typography as="p" pb="2" weight={600}>breakpointKeys: {breakpointKeys}</Typography>
            <Typography as="p" pb="2" weight={600}>current: {current}</Typography>
            <Typography as="p" pb="2" weight={600}>innerWidth: {innerWidth}</Typography>
        </StyledResponsiveContextExample>
    );
};
```
