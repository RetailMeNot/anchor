...
// COMPONENT
import { Adaptor, LegacyBreakpoints, StandardBreakpoints } from './Adaptor.component';

const { LegacyDesktop, LegacyPhone, LegacyTablet, XS, SM, MD, LG, XL, XXL, CustomAdaptor } = Adaptor;
...
const dimension = ({ min, max }: { min?: number; max?: number }): string => {
  ...
  return `(${min ? `${min}px` : `${notSet}`} - ${max ? `${max}px` : `${notSet}`})`;
};
...
storiesOf('Components/Grid/Adaptor', module)
  .add('Legacy Breakpoints', () => (
    <StyledStory>
      ...
      <LegacyDesktop>
        <p>
          I am visible on Desktop {dimension(LegacyBreakpoints.desktop)}
        </p>
      </LegacyDesktop>
      <LegacyTablet>
        <p>
  ...
  .add('Custom Breakpoints', () => (
    <StyledStory>
      ...
      <CustomAdaptor minWidth={number('Minimum Width', 1)} maxWidth={number('Maximum Width', 99999)}>
